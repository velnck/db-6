...
BEGIN 
    ...
    IF v_type_check = 'SELECT' 
    THEN 
        v_n_json_array := v_nest_json_data.get_array('tables');
        FOR j IN 0..v_n_json_array.get_size() - 1 
        LOOP 
            IF v_tab IS NULL 
            THEN 
        ...
        LOOP 
            ...
            IF f_n_q_type = 'plain'
                OR f_n_q_type IS NULL 
            THEN 
                f_n_cond := v_n_temp_obj.get_string('condition');
                IF v_filter_cond IS NULL 
                THEN 
...
--                    DBMS_OUTPUT.PUT_LINE('1' || f_n_operator || '1');
                ...
                END IF;
            ELSIF f_n_q_type = 'included' 
            THEN 
                f_n_cond := process_select_condition
                (
...
BEGIN 
    ...
    THEN 
        ...
        LOOP 
            ...
            END IF;
        END LOOP;
        DBMS_OUTPUT.PUT_LINE('SELECT query statement');
        v_sql := 'SELECT ' || v_columns || ' FROM ' || v_tables;
        IF v_join_conditions IS NOT NULL 
        THEN 
            v_sql := v_sql || ' ' || v_join_conditions || ' WHERE ';
        END IF;
        IF v_filter_conditions IS NOT NULL 
        THEN 
            IF v_join_conditions IS NULL 
            THEN 
    ...
    THEN 
        ...
        ELSE 
            v_filter_conditions := process_select_condition(
                TREAT(
    ...
    ELSIF v_query_type = 'UPDATE' THEN 
        ...
        LOOP 
            ...
            IF v_set_type IS NULL 
            THEN 
                IF i != v_json_array.get_size() - 1 
                THEN 
        ...
        EXECUTE IMMEDIATE v_sql;
    ELSIF v_query_type = 'CREATE TABLE' 
    THEN 
        DBMS_OUTPUT.PUT_LINE('CREATE TABLE statement');
        SELECT json_value(json_str, '$.table') INTO v_tables
        FROM dual;
        SELECT LISTAGG(column_name || ' ' || data_type, ', ') WITHIN GROUP (
        ...
        IF v_pks IS NOT NULL 
        THEN 
            v_sql := v_sql || ', ' || v_pks;
        END IF;
        IF v_fks IS NOT NULL 
        THEN 
            v_sql := v_sql || ', ' || v_fks;
        END IF;
        v_sql := v_sql || ')';
...
END;';
        DBMS_OUTPUT.PUT_LINE(v_sql);
        EXECUTE IMMEDIATE v_sql;
        DBMS_OUTPUT.PUT_LINE('Table ' || v_tables || ' created.');
    ELSIF v_query_type = 'DROP TABLE' 
    THEN 
        v_drop_param := v_json_data.get_string('parameters');
        SELECT json_value(json_str, '$.table') INTO v_tables
        FROM dual;
        v_sql := 'DROP TABLE ' || v_tables;
        IF v_drop_param = 'purge' 
        THEN 
            v_sql := v_sql || ' purge';
        END IF;
        EXECUTE IMMEDIATE v_sql;
